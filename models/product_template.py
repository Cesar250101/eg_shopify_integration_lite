...
class ProductTemplate(models.Model):
                ...
                while count > 0:
                    ...
                    if response:
                        ...
                        for product in product_list:
                            ...
                            if 'image' in product:
                                try:
                                    imagen=image = base64.b64encode(requests.get(product['image']['src'].replace('\/', '/')).content)
                                except:
                                    imagen=""    
                            else:
                                imagen=""
    ...
    def create_product_variant_at_import(self, product=None, instance_id=None, product_tmpl_id=None,
        ...
        for product_variant in product.get("variants"):
            # value_list = []
            eg_product_id = None
            # if not check attribute means method not call for only compare attribute
            ...
            # if not get mapping product variant so find odoo product and create mapping
            if not eg_product_id:
                attribute_value_ids = self.env["product.attribute.value"]
                len_options = len(product.get("options"))

                for count in range(len_options):
                    attribute_name = product.get("options")[count].get("name")
                    # if method call by remaining product variant so attribute get from new_option_list so skip attribute
                    if new_option_list:
                        if attribute_name in new_option_list:
                            continue
                    # method call by odoo product available but not get mapping so compare attribute and value if same so mapping
                    ...
                    if product_variant.get(option):
                        ...
                        attribute_value_ids += self.env["product.attribute.value"].search(
                            [("name", "=", product_variant.get(option)),
                             ("attribute_id.name", "=", attribute_name)])
                # attribute_value_ids = self.env["product.attribute.value"].search(
                #     [("name", "in", value_list), ("attribute_id.name", "in", attribute_list)])
                product_id = product_tmpl_id.product_variant_ids.filtered(  # TODO for remove new attribute value
                    lambda l: l.attribute_value_ids == attribute_value_ids)
                # if odoo product variant so mapping of product
                if product_id:
                    # if method call for only compare attribute so don't_mapping
                    if check_attribute:
                        continue
                    uom_category_id = self.env["uom.category"].search([("name", "=", "Weight")],
                                                                      limit=1)
                    ...
                    eg_value_ids = self.env["eg.attribute.value"].search(
                        [("odoo_attribute_value_id", "in", product_id.attribute_value_ids.ids),
                         ("instance_id", "=", instance_id.id)])
